 ...
 * @note 
 1、 请勿修改本框架代码的函数定义
 2、 编程版本选择C或者C++其中一种即可，对应框架代码逻辑没有变化。
...
int main() {
	...
	for (j = 2; j < max_memory_size; ++j) {
        printf("when cache size is %d \n", j);

		reset_data(output, data_num);
        rate = FIFO(data, data_num, j, output);
        printf("The hit rate of FIFO is:  %.3f \n", rate);
        printf("The output in FIFO is: \n");
		show_output(output, data_num);

		reset_data(output, data_num);
        rate = LRU(data, data_num, j, output);
        printf("The hit rate of LRU is:  %.3f \n", rate);
        printf("The output in LRU is: \n");
		show_output(output, data_num);

		reset_data(output, data_num);
		rate = optimal(data, data_num, j, output);
        printf("The hit rate of optimal is:  %.3f \n", rate);
        printf("The output in optimal is: \n");
		show_output(output, data_num);
